...
RCT_EXPORT_VIEW_PROPERTY(onConferenceRecordStart, RCTBubblingEventBlock)
RCT_EXPORT_VIEW_PROPERTY(onConferenceRecordEnd, RCTBubblingEventBlock)
...
RCT_EXPORT_METHOD(navigateScreen)
{
    dispatch_sync(dispatch_get_main_queue(), ^{
...
- (void)recordStarted:(NSDictionary *)data {
    RCTLogInfo(@"Conference Record Start");
    if (!jitsiMeetView.onConferenceRecordStart) {
        return;
    }

    jitsiMeetView.onConferenceRecordStart(data);
}

- (void)recordEnded:(NSDictionary *)data {
    RCTLogInfo(@"Conference Record End");
    if (!jitsiMeetView.onConferenceRecordEnd) {
        return;
    }

    jitsiMeetView.onConferenceRecordEnd(data);
}
@end
